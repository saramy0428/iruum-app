...
import { computePillarsFromInput } from "./computePillars.js";
...
export async function generateDestinyName(input, options = {}) {
  ...
  const { surname, birthDate, birthTime, birthCountry, birthCity, placeUnknown, gender } = input;
  const { sessionSeed: seedOverride = null, topN = 3, explorationBase = 0.15 } = options;

  // ── 1. Saju computation ──────────────────────────────────────────────────
  const fourPillars = computePillarsFromInput({ birthDate, birthTime, birthCountry, placeUnknown });
